...
func TestCircleLifecycle(t *testing.T) {
	...
	t.Run("Step4_RecordContribution", func(t *testing.T) {
		contribRepo.On("FindByTxnHash", mock.Anything, mock.AnythingOfType("string")).Return(nil, nil).Once()
	...
	t.Run("Step5_RecordPayout", func(t *testing.T) {
		circleID := uuid.New()
		payoutRepo.On("FindByTxnHash", mock.Anything, mock.AnythingOfType("string")).Return(nil, nil).Once()
